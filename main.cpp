...
void left_right_controller(struct program_state* state) {
    ...
    while (state->running) {
        ...
        if (a[0] == 27 && a[1] == 91) {
            switch (a[2]) {
                case 67: result = state->falling_shape->Right(state->shapes); break;
                case 68: result = state->falling_shape->Left(state->shapes); break;
            }
        }
    }
};
...
int main() {
    ...
    while (true) {
        ...
        if (!state->falling_shape->Fall(state->shapes)) {
            new_random_shape(state);
        }

        // If the shape now overlaps a shape, undo the fall and make a new shape
        if (overlaps_shape(state->shapes, state->falling_shape)) {
            state->falling_shape->y--;
            new_random_shape(state);
        }
